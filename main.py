...
parser.add_argument('--output',default='../output_ours_rgb')
...
last_seq = None
pred_masks = None
with DavisInteractiveSession(davis_root=davis_path+'/trainval', report_save_dir=out_path, max_nb_interactions=8, max_time=8*30) as sess:
    while sess.next():
        ...
        if new_seq:
            ...
            processor = DAVISProcessor(config, xmem, fusion_model, s2m_model, images[sequence], num_objects[sequence])
            print(sequence)
