...
from util.tensor_util import unpad_3dim,unpad_2dim
...
class App(QWidget):
    def __init__(self,config, xmem, s2m_ctrl: S2MController, fbrs_ctrl: FBRSController,
                 images, masks, num_objects):
        super().__init__()
        ...
        img_buff,self.mid_pad=images_to_torch(images, device='cpu')
        self.processor = InferenceCore(config, xmem, fusion_model,img_buff, config['num_objects'], mem_profile=0,
                      device='cuda:0')
    ...
    def on_run(self):
        ...
        self.current_mask = unpad_3dim(self.processor.interact(self.interacted_mask, self.cursur,
                                                    self.progress_total_cb, self.progress_step_cb),self.mid_pad)
        self.interacted_mask = None
    ...
    def update_interacted_mask(self):
        if self.in_local_mode:
            ...
            self.current_mask[self.cursur] = unpad_2dim(self.processor.np_masks[self.cursur],self.mid_pad)
        self.show_current_frame()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--images', default=r'C:\Users\chen\Desktop\test231231',
    ...
    parser.add_argument('--num_objects', default=5,help='Default: 1 if no masks provided, masks.max() otherwise', type=int)
    ...
    with torch.cuda.amp.autocast(enabled=not args.no_amp):
        ...
        ex = App(config, xmem, s2m_controller, fbrs_controller,
                 images, masks, num_objects)
        sys.exit(app.exec_())
