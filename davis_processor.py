...
class DAVISProcessor:
    ...
    def __init__(self, config, xmem, fuse_net, s2m_net, images, num_objects, device='cuda:0'):
        ...
        self.processor = InferenceCore(config, xmem, fuse_net, images, num_objects, mem_profile=0, device=device)
    ...
    def to_mask(self, scribble):
        ...
        for ki in range(1, self.k + 1):
            ...
            # print((self.processor.masks[idx] == ki).to(self.device).float().unsqueeze(0))
            # print(((self.processor.masks[idx] == ki).to(self.device).float().unsqueeze(0)).shape)
    ...
    def interact(self, scribble, gui_call = False):
        # if not gui_call:
        #     mask, idx = self.to_mask(scribble)
        # else:
        #     mask = scribble['mask']
        #     idx = scribble['idx']
        #     self.davis_schedule=[0,1,2,3,4,5,6,7]
        mask, idx = self.to_mask(scribble)
